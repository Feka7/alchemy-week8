...
describe("Casino", async () => {
  ...
  it("Not allow you to reveal first number with wrong value", async () => {
    f = await ethers.getContractFactory("Casino")
    c = await f.deploy()
    ...
    expect(rcpt.events[0].event).to.equal("BetAccepted")
    try {
      tx = await c.revealHalf(hashA, valBlose)
      rcpt = await tx.wait()   
      expect("this").to.equal("fail")      
    } catch (err) {
        expect(interpretErr(err)).to
  ...
  it("Not allow you to reveal the second number with wrong value", async () => {
    f = await ethers.getContractFactory("Casino")
    c = await f.deploy()
    ...
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetAccepted")
    tx = await c.revealHalf(hashA, valBwin)
    rcpt = await tx.wait()
  ...
  it("Not allow you to reveal beofre first number is revealed", async () => {
    f = await ethers.getContractFactory("Casino")
    c = await f.deploy()
    ...
    try {
      ...
      expect("this").to.equal("fail")      
        ...
        expect(interpretErr(err)).to
          .match(/Bet has not been revealed yet/)
    }          
  })
  ...
  it("Work all the way through (B wins)", async () => {
    ...
    tx = await cB.revealHalf(hashA, valBwin)
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetRevealed")
  ...
  it("Work all the way through (A wins)", async () => {
    ...
    tx = await cB.revealHalf(hashA, valBlose)
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetRevealed")
