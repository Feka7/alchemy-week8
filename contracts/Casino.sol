...
contract Casino {
    ...
    event BetRevealed(uint256 indexed _commitment, address indexed _sideA);
    ...
    function acceptBet(uint256 _commitment, uint256 _random) external payable {
        ...
        acceptedBet[_commitment].revealed = false;
        proposedBet[_commitment].accepted = true;

        emit BetAccepted(_commitment, proposedBet[_commitment].sideA);
    } // function acceptBet

    //Called by sideB after acceptBet. This mechanism prevents frontrunning attack 
    function revealHalf(uint256 _commitment, uint256 _random) external {
      uint256 _randomRevealed = uint256(keccak256(abi.encodePacked(_random)));
      require(
          acceptedBet[_commitment].sideB == msg.sender,
          "Not a bet you placed"
      );
      require (
        acceptedBet[_commitment].randomB == _randomRevealed,
        "Wrong value"
      );
      acceptedBet[_commitment].randomB = _random;
      acceptedBet[_commitment].revealed = true;
      emit BetRevealed(_commitment, proposedBet[_commitment].sideA);
    }
    ...
    function reveal(uint256 _random) external {
        ...
        require(
            acceptedBet[_commitment].revealed,
            "Bet has not been revealed yet"
        );
